...
public class MenuManager : MonoBehaviour
{
    ...
    private int userScore = 50; // Initial value is taken as 0
    ...
    public async void Post_Score_On_Leaderboard()
    {
        try
        {
            if (string.IsNullOrEmpty(scoreInputField.text)) return;
            ...
            // Else set the score
            await scoreObj.AddScore(this.userScore);
        }
        catch (Exception e)
        {
    ...
    private async Task GenerateLeaderBoard()
    {
        try
        {
            ...
            RootResponseLeaderboard res = await Lobj.FetchRecords("Global");
            ...
            if ( res.data.records.Count == 0)
            {
                Debug.Log("Records fetched Succefully but you have no record at this time in leaaderboard");
            }
